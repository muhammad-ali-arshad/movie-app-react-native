...
export default function MovieScreen() {
  ...
  const item = params?.item ? params.item : params
...
useEffect(() => {
  if (!item || !item.id) {
    console.log('[MovieScreen] No valid item.id found in route params:', params)
    return
  }
  setLoading(true)
...
const getSimilarMovies = async id => {
  ...
  console.log('[MovieScreen] got similar movies:', data)
  if (data && Array.isArray(data.results)) {
    console.log('[MovieScreen] similar movies length:', data.results.length)
    setSimilarMovies(data.results)
  } else {
    console.log('[MovieScreen] similar movies missing or invalid')
    setSimilarMovies([])
  }
}
